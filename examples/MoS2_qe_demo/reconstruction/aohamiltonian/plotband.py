...
def loaddata(folder, filename):
    ...
    if filename.endswith('.json'):
        ...
        if os.path.isfile(f'{folder}/lat.dat'):
            rprim = np.loadtxt(f'{folder}/lat.dat').T / bohr2ang
        elif os.path.isfile(f'{folder}/POSCAR'):
            with open(f'{folder}/POSCAR', 'r') as f:
                f.readline(); f.readline()
                rprim = np.empty((3, 3))
                for i in range(3):
                    rprim[i] = list(map(float, f.readline().split()))
        else:
            raise RuntimeError('Cannot find structure information')
