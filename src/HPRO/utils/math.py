...
from ..config import SPHERICAL_HARMONICS_LMAX
...
'''
def compute_cg_spinor(l, j_gt_l):
    m = np.arange(-l, l+1, 1, dtype=np.float64)
    cg = np.zeros((2, 2*l+1), dtype=np.float64)
    if j_gt_l:
        cg[0, :] = np.sqrt((l + m + 1) / (2 * l + 1))
        cg[1, :] = np.sqrt((l - m + 1) / (2 * l + 1))
    else:
        cg[0, :] = - np.sqrt((l - m) / (2 * l + 1))
        cg[1, :] = np.sqrt((l + m) / (2 * l + 1))
    return cg

def compute_ylm_cplx_to_real(l):
    # the phi part is (sin(l*phi), sin((l-1)*phi), ..., sin(phi), 1, cos(phi), cos(2*phi), ..., cos(l*phi))
...
def get_cg_spinor(l: int, j: float):
    """
    Get CG coefficients of < l m_l 1/2 m_s | j m_j > 

    Returns:
        cg array(2, 2*l+1): CG coefficients, first dimension is spinor up & down, second dimension is angular momentum m_l
    """
    if abs(j - l - 0.5) < 1e-6:
        return cg_spinor_up[l]
    elif abs(j - l + 0.5) < 1e-6:
        return cg_spinor_dn[l]
    else:
        raise ValueError(f"invalid value: l = {l}, j = {j}")

cg_spinor_up = []
for l in range(SPHERICAL_HARMONICS_LMAX):
    cg_spinor_up.append(compute_cg_spinor(l, True))

cg_spinor_dn = []
for l in range(SPHERICAL_HARMONICS_LMAX):
    cg_spinor_dn.append(compute_cg_spinor(l, False))
    
ylm_cplx_to_real = []
for l in range(SPHERICAL_HARMONICS_LMAX):
    ylm_cplx_to_real.append(compute_ylm_cplx_to_real(l))


def get_dmat_coeffs(l, j):
    """
    Compute D matrix coefficients:
    \sum_{m_j} < l m_l 1/2 m_s | j m_j > < j m_j | l m_l' 1/2 m_s' >
    and then convert to real spherical harmonics basis

    Returns:
        dcoeffs array(2, 2, 2*l1+1, 2*l2+1): (m_s, m_s', m_l, m_l')
    """
    cg = get_cg_spinor(l, j)
    dcoeffs = cg[:, None, :, None] * cg[None, :, None, :].conj()
    # enforce m_l + m_s = m_l' + m_s'
    for isp in range(2): 
        for jsp in range(2):
            dcoeffs[isp, jsp, ...] *= np.eye(2*l+1, k=jsp-isp)
    U = ylm_cplx_to_real[l]
    dcoeffs = np.matmul(np.matmul(U, dcoeffs), U.conj().T)
    return dcoeffs
...
def spbessel_transfrorm(l, k, rgd, R_r, norm='forward'):
    ...
    sbt_real = rgd.integrate(j_l*R_r)
    if norm=='forward':
        sbt_real *= 4.0*np.pi
        cplx_phase = (-1j)**l
    elif norm=='backward':
